...
export function Chat() {
  ...
  function handleSend(e: React.FormEvent) {
    ...
    const fullPrompt = buildConversationPrompt(history, text);
  ...
  function handleJobDone(result: string, latencyMs: number | null) {
    ...
    if (result.startsWith("__error__:")) {
      setMessages((prev) => [...prev, { id: crypto.randomUUID(), role: "error", content: result.slice(10) }]);
    } else {
      setMessages((prev) => [...prev, { id: crypto.randomUUID(), role: "assistant", content: result, latencyMs: latencyMs ?? undefined }]);
    }
  }

  function handleClear() {
    setMessages([{ id: "welcome", role: "assistant", content: "Conversation cleared. How can I help you?" }]);
    setPendingJobId(null);
  }
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 sm:px-6 py-2.5 border-b border-border bg-card shrink-0 gap-2">
        ...
        <div className="flex items-center gap-1.5 flex-1 justify-end">
          ...
          <button onClick={handleExport} className="p-2 rounded border border-border text-muted-foreground hover:text-primary hover:border-primary/40 transition-colors shrink-0" title="Export conversation">
            <Download className="w-3.5 h-3.5" />
          </button>
          <button onClick={handleClear} disabled={isWaiting} className="p-2 rounded border border-border text-muted-foreground hover:text-destructive hover:border-destructive/40 transition-colors disabled:opacity-40 shrink-0" title="Clear conversation">
      ...
      <div className="flex-1 overflow-y-auto px-3 sm:px-6 py-4 sm:py-6 space-y-4">
        <AnimatePresence initial={false}>
          {messages.map((msg) => (
            <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
              <div className={`w-7 h-7 rounded-full shrink-0 flex items-center justify-center border ${msg.role === "user" ? "bg-primary/10 border-primary/30" : msg.role === "error" ? "bg-destructive/10 border-destructive/30" : "bg-secondary/10 border-secondary/30"}`}>
                {msg.role === "user" ? <User className="w-3.5 h-3.5 text-primary" /> : msg.role === "error" ? <AlertCircle className="w-3.5 h-3.5 text-destructive" /> : <Bot className="w-3.5 h-3.5 text-secondary" />}
              </div>
              <div className={`max-w-[85%] sm:max-w-[80%] rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3 ${msg.role === "user" ? "bg-primary text-primary-foreground rounded-tr-sm" : msg.role === "error" ? "bg-destructive/10 border border-destructive/30 text-destructive rounded-tl-sm" : "bg-card border border-border rounded-tl-sm"}`}>
                {msg.role === "user" ? (
                ...
                {msg.latencyMs && <p className="text-[10px] text-muted-foreground font-mono mt-2 opacity-60">{msg.latencyMs}ms</p>}
              </div>
            </motion.div>
          ))}
          {isWaiting && (
            <motion.div key="thinking" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex gap-3">
      ...
      {/* Input */}
      <div className="shrink-0 border-t border-border bg-card px-3 sm:px-6 py-3 sm:py-4">
        {!llmKey && provider !== "0g" && (
          <div className="flex items-center gap-2 mb-3 px-3 py-2 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
            <Zap className="w-3.5 h-3.5 text-yellow-400 shrink-0" />
            <p className="text-xs text-yellow-400 font-mono">Set your {provider.toUpperCase()} API key in Settings.</p>
          </div>
        )}
        <form onSubmit={handleSend} className="flex gap-2 sm:gap-3">
          <Textarea
            ...
            placeholder="Ask about tokenomics, 0G ecosystem... (Enter to send)"
            className="flex-1 font-mono text-sm resize-none min-h-[44px] max-h-[160px] bg-background border-border"
            disabled={isWaiting}
            rows={1}
          />
          <Button type="submit" disabled={isWaiting || !input.trim()} className="shrink-0 font-mono px-3 sm:px-4 h-auto">
